...
// TestVerifyPeerChain_FailsClosed: nil pool and empty peer chain are
// each rejected before any chain building happens.
func TestVerifyPeerChain_FailsClosed(t *testing.T) {
	if err := VerifyPeerChain(nil, tls.ConnectionState{}); err == nil {
		t.Error("expected error for nil pool")
	}
	if err := VerifyPeerChain(x509.NewCertPool(), tls.ConnectionState{}); err == nil {
